...
function DewHeaterCard({ showToast }) {
  ...
  return (
    <Card title="Dew heater"
      ...
      <div className="mt-4 rounded-lg border border-amber-800 bg-amber-950/60 p-3">
        <p className="text-sm font-semibold text-amber-300">
          ⚠ Experimental — can brown out the camera
        </p>
        <p className="mt-1 text-xs text-amber-200/80">
          The heater draws real current from the camera’s power supply. On a
          supply without headroom that extra amp causes voltage dips, and can
          shut the Pi down hard — a red light and a camera that stays dead
          until the power is physically unplugged and reconnected. This has
          happened, repeatedly, on a supply that looked more than big enough
          on paper. Only run the heater from a supply with real margin, or
          power the heater from its own separate supply. If your camera dies
          overnight with the heater on, this is the first suspect.
        </p>
      </div>
 ...
 *
 * Un-parked 2026-09-15, after the full flow — install, QR login, HTTPS serve,
 * dashboard loading on a phone over cellular — was finally seen working on a
 * real camera. It had failed on hardware twice before that; the cause (an
 * apt update scoped to one source resolving against stale Debian indexes)
 * only surfaced once the rig's own journal became readable remotely, and the
 * card spent the intervening month saying "coming soon" rather than offering
 * a button that fails. If this ever regresses, that is the bar for turning it
 * back off: no dead buttons.
 *
 * Every state here carries an action or an explanation; none of them just
 * report a fact and stop — "Tailscale isn't installed" comes with the install
 * button, not as a dead end.
 */
function RemoteCard({ showToast }) {
  ...
  const post = async (path, working, done) => {
    ...
    if (!r?.ok) {
      const detail = await r?.json().catch(() => null)
      showToast?.(errorText(detail, 'That didn’t work — see the card for details'))
    } else if (done) {
      showToast?.(done)
    }
    refresh()
        ...
        <div className="mt-3 space-y-3">
          <div className="rounded-lg bg-emerald-950 p-3 text-sm">
            ...
            <p className="mt-2 text-xs text-zinc-400">
              ...
              certificate warnings — the very first visit can take half a
              minute while the certificate is issued.
            </p>
          </div>
          ...
          {remote.can_install ? (
            <>
              ...
              <p className="text-xs text-zinc-500">
                ...
                camera needs internet access. It refreshes the camera’s package
                lists first, so allow a few minutes — capture keeps running
                throughout.
              </p>
            </>
          ) : (
